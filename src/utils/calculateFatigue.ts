...
export const calculatePerformanceOverTime = (inputs: TourInputs, totalHours: number): PerformancePoint[] => {
  ...
  for (let hour = 0; hour <= Math.ceil(totalHours); hour++) {
    ...
    const terrainFatigue = inputs.terrain === 'alpine_extreme' ? hour * 0.5 : 
                          inputs.terrain === 'alpine_hard' ? hour * 0.3 : 
                          inputs.terrain === 'alpine_medium' ? hour * 0.2 : 0;
    
    // Calculate total fatigue and convert to performance (100% - fatigue%)
    let totalFatigue = (baseFatigue + packageFatigue + terrainFatigue) * (1 / baseMultiplier);
