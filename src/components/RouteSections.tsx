    ...
    <div className="bg-white p-3 sm:p-4 rounded-lg shadow mb-4 sm:mb-6">
      ...
      <div className="space-y-2">
        {sections.map((section, index) => (
          <div
            ...
            style={{ borderLeftWidth: '4px', borderLeftColor: SECTION_COLORS[index % SECTION_COLORS.length] }}
          >
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              ...
              <div className="flex-shrink-0">
                <select
                  ...
                  onChange={(e) =>
                    onUpdateSectionTerrain(section.id, e.target.value as Terrain)
                  }
                ...
                >
                  ...
                  <option value="TECHNICAL_ALPINE">{t('terrain_technical_alpine')}</option>
                </select>
              </div>
            </div>
          </div>
      ...
      <div className="mt-3 p-2 bg-gray-50 rounded text-xs text-gray-500">
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          <span>
            {t('gpxTotalDistance')}: {sections.reduce((sum, s) => sum + s.horizontalDistance, 0).toFixed(2)} km
          </span>
          <span className="flex items-center gap-1">
            <FaArrowUp className="text-green-600" />
